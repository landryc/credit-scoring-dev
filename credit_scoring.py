...
def predict_logic():
	...
	infos = compute_score(elt, *model, app_version)
	return infos
...
def normalize(elt, norm_params, app_version):
	for criterion in norm_params.keys():
		if criterion == 'Due date - repayment date' and app_version:
			continue 
		else:
...
def compute_score(elt, Options, weights, Min, Max, norm_params, app_version):
	score = 0
	row = normalize(elt, norm_params, app_version)
